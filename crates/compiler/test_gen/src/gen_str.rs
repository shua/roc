...
fn str_from_utf8_lossy_expected_continuation() {
    assert_evals_to!(
        r#"Str.fromUtf8Lossy [97, 98, 0xC2, 99]"#,
...
fn str_from_utf16() {
    assert_evals_to!(
        indoc!(
            r#"
                when Str.fromUtf16 [0x72, 0x6f, 0x63] is
                    Ok val -> val
                    _ -> ""
            "#
        ),
...
fn str_from_utf16_emoji() {
    assert_evals_to!(
        indoc!(
            r#"
                when Str.fromUtf16 [0x72, 0xd83d, 0xdc96, 0x63] is
                    Ok val -> val
                    _ -> ""
            "#
        ),
...
fn str_from_utf16_err_expected_second_surrogate_half() {
    assert_evals_to!(
        indoc!(
            r#"
                when Str.fromUtf16 [0x72, 0xd83d, 0x63] is
...
fn str_from_utf16_err_unexpected_second_surrogate_half() {
    assert_evals_to!(
        indoc!(
            r#"
                when Str.fromUtf16 [0x72, 0xdc96, 0x63] is
                    Err (BadUtf16 {problem: EncodesSurrogateHalf, index: index }) -> index
                    _ -> 42
            "#
        ),
...
fn str_from_utf16_lossy() {
    assert_evals_to!(
        r#"Str.fromUtf16Lossy [0x72, 0xdc96, 0x63]"#,
        roc_std::RocStr::from("r�c"),
        roc_std::RocStr
    )
}
...
fn str_from_utf32() {
    assert_evals_to!(
        indoc!(
            r#"
                when Str.fromUtf32 [0x72, 0x6f, 0x63] is
                    Ok val -> val
                    _ -> ""
            "#
        ),
...
fn str_from_utf32_emoji() {
    assert_evals_to!(
        indoc!(
            r#"
                when Str.fromUtf32 [0x72, 0x1f496, 0x63] is
                    Ok val -> val
                    _ -> ""
            "#
        ),
...
fn str_from_utf32_err_codepoint_too_large() {
    assert_evals_to!(
        indoc!(
            r#"
                when Str.fromUtf32 [0x72, 0x123456, 0x63] is
...
fn str_from_utf32_err_utf8_cannot_encode_surrogate_half() {
    assert_evals_to!(
        indoc!(
            r#"
                when Str.fromUtf32 [0x72, 0xd83d, 0x63] is
...
fn str_from_utf32_lossy() {
    assert_evals_to!(
        r#"Str.fromUtf32Lossy [0x72, 0x123456, 0x63]"#,
        roc_std::RocStr::from("r�c"),
        roc_std::RocStr
    )
}
