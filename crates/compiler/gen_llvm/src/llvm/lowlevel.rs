...
pub(crate) fn run_low_level<'a, 'ctx>(
    ...
    match op {
        ...
        StrFromUtf8 => {
            // Str.fromUtf8Lowlevel : List U8 -> FromUtf8Result
            arguments!(list);
        ...
        StrFromUtf16 => {
            // Str.fromUtf16Lowlevel : List U16 -> FromUtf16Result
            arguments!(list);
            call_str_from_utf_bitcode_fn(
                env,
                layout_interner,
                &[list],
                "str.FromUtf16Result",
                bitcode::STR_FROM_UTF16,
            )
        }
        StrFromUtf32 => {
            // Str.fromUtf32Lowlevel : List U32 -> FromUtf32Result
            arguments!(list);
            call_str_from_utf_bitcode_fn(
                env,
                layout_interner,
